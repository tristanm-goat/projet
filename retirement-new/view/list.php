<ul class="facility-list">
    <li class="facility-item">
        <span class="like-icon">&hearts;</span>
        <h3>Sunny Meadows Retirement Community</h3>
        <p><strong>Address:</strong> 123 Golden Years Ave, Pleasantville</p>
        <p>A vibrant community with a wide range of activities and excellent care services.</p>
    </li>
    <li class="facility-item">
        <span class="like-icon">&hearts;</span>
        <h3>Lakeside Manor</h3>
        <p><strong>Address:</strong> 456 Serenity Rd, Waterview</p>
        <p>Enjoy beautiful lake views and a peaceful environment with top-notch amenities.</p>
    </li>
    <li class="facility-item">
        <span class="like-icon">&hearts;</span>
        <h3>The Oaks Assisted Living</h3>
        <p><strong>Address:</strong> 789 Liberty Ln, Freedom Town</p>
        <p>Personalized care plans in a supportive and friendly atmosphere.</p>
    </li>
    <li class="facility-item">
        <span class="like-icon">&hearts;</span>
        <h3>Pine Ridge Gardens</h3>
        <p><strong>Address:</strong> 101 Forest Dr, Woodsville</p>
        <p>Nestled in a quiet, wooded area, offering a tranquil retirement experience.</p>
    </li>
    <li class="facility-item">
        <span class="like-icon">&hearts;</span>
        <h3>Harborview Senior Living</h3>
        <p><strong>Address:</strong> 212 Bay St, Port City</p>
        <p>Active living for seniors with a focus on health, wellness, and social engagement.</p>
    </li>
    <li class="facility-item">
        <span class="like-icon">&hearts;</span>
        <h3>Maplewood Estates</h3>
        <p><strong>Address:</strong> 333 Maple Ave, Grove Heights</p>
        <p>Luxurious apartments and a full calendar of events for an active lifestyle.</p>
    </li>
</ul>
